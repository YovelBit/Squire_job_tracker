...
document.addEventListener("DOMContentLoaded", () => {
    ...
    (response) => {
        ...
        if (response.success) {
            ...
            statusMessage.textContent = "Failed to save job: " + JSON.stringify(response.error);
            statusMessage.className = "error";
        }
    }
);
...
document.addEventListener("DOMContentLoaded", () => { // When popup is loaded
    ...
    settingsForm.addEventListener("submit", (e) => { // On form submit, rather than button click, easier for automation
        e.preventDefault();
        ...
        chrome.storage.sync.set({ // Storing credentials securely on chrome storage
            apiBaseUrl,
            email,
            password
        }, () => {
